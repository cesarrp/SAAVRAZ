...
def plotSegmentationResults(flagsInd, flagsIndGT, classNames, mtStep, ONLY_EVALUATE=False):
    ...
    if not ONLY_EVALUATE:
        ...
        createdate = str("resultadoSegmentacao_") + str(datetime.datetime.now()).split(' ')[1] + str(".png")
...
def silenceRemoval(x, Fs, stWin, stStep, smoothWindow=0.5, Weight=0.5, plot=False):
    ...
    if plot:
        ...
        ## ima change this beach: was plt.show()
        createdate = str("probabilidadeSVM_") + str(datetime.datetime.now()).split(' ')[1] + str(".png")
        plt.savefig(createdate, bbox_inches='tight')
        # plt.show()
...
def speakerDiarization(fileName, numOfSpeakers, mtSize=2.0, mtStep=0.2, stWin=0.05, LDAdim=35, PLOT=False):
    ...
    if PLOT:
        ...
        ## ima change this beach: was plt.show()
        createdate = str("cluster_") + str(datetime.datetime.now()).split(' ')[1] + str(".png")
        plt.savefig(createdate, bbox_inches='tight')
        # plt.show()
    return cls
